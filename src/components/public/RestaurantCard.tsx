...
export const RestaurantCard = ({ business }: RestaurantCardProps) => {
  ...
  return (
    <Link to={`/business/${business.id}`}>
      <Card className="flex flex-row overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 h-full">
        ...
        <div className="w-1/3 relative aspect-square">
          ...
          <Button
            size="icon"
            variant="ghost"
            className="absolute top-2 right-2 bg-background/20 hover:bg-background/40 backdrop-blur-sm h-9 w-9 rounded-full"
            onClick={toggleFavorite}
          >
            <Heart
              className={`h-4 w-4 transition-colors drop-shadow-md ${
