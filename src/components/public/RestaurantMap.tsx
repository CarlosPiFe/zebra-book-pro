...
export const RestaurantMap = ({ businesses, onBusinessClick, userLocation }: RestaurantMapProps) => {
  ...
  useEffect(() => {
    ...
    // Add markers using Mapbox Geocoding API when needed (address preferred)
    let disposed = false;
    ...
    const addMarkers = async () => {
      ...
      await Promise.all(
        businesses.map(async (business) => {
          ...
          // Resolve coordinates: prefer geocoding by address, fallback to stored lat/lng
          let coords: [number, number] | null = null;
          const addr = business.address?.trim();
          if (addr) {
            const cacheKey = `${business.id}|${addr}`;
            const cached = geocodeCacheRef.current.get(cacheKey);
            if (cached) {
              coords = cached;
            } else {
              coords = await geocodeAddress(addr);
              if (coords) geocodeCacheRef.current.set(cacheKey, coords);
            }
          }
          if (!coords) {
            const latNum = Number(business.latitude);
            const lngNum = Number(business.longitude);
            if (isFinite(latNum) && isFinite(lngNum)) coords = [lngNum, latNum];
          }
          if (!coords || !isFinite(coords[0]) || !isFinite(coords[1])) {
            console.warn('⚠️ Sin coordenadas válidas para', business.name, business.address);
            return;
          }
          ...
          const popup = new mapboxgl.Popup({ offset: 25, closeButton: true, closeOnClick: false, maxWidth: 'none' }).setDOMContent(cardEl);
  ...
  useEffect(() => {
    ...
    const addMarkers = async () => {
      ...
      if (coordsList.length > 1) {
        if (userLocation) bounds.extend([userLocation.lng, userLocation.lat]);
        map.current.fitBounds(bounds, { padding: 50 });
      } else if (coordsList.length === 1 && !userLocation) {
        const only = coordsList[0];
        if (only) map.current.setCenter(only);
      }
    };
